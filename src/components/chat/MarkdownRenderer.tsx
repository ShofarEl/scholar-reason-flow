...
// Normalize KaTeX sizing and contrast in chat
const chatMathStyles = `
  .katex { font-size: 1em !important; }
  .dark .katex { color: rgb(248 250 252) !important; }
`;
if (typeof document !== 'undefined' && !document.getElementById('katex-chat-styles')) {
  const styleElement = document.createElement('style');
  styleElement.id = 'katex-chat-styles';
  styleElement.textContent = chatMathStyles;
  ...
  return (
    <div
      ...
      className={[
        ...
        isUser
          ? 'prose-invert prose-headings:text-white prose-p:text-white prose-strong:text-white prose-em:text-white prose-code:text-white'
          : theme === 'dark'
            ? 'dark:prose-invert prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-em:text-foreground prose-code:text-foreground'
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          code: ({ node, className, children, ...props }: any) => {
            ...
            return (
              <code
                className={`px-1.5 py-0.5 rounded-md text-[0.85em] font-mono border ${
                  isUser
          ...
          ul: ({ children }) => (
            <ul className={`list-disc ml-6 my-3 space-y-1 ${isUser ? 'text-white' : 'text-foreground'}`}>
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol className={`list-decimal ml-6 my-3 space-y-1 ${isUser ? 'text-white' : 'text-foreground'}`}>
              {children}
            </ol>
          ),
          li: ({ children }) => (
            <li className={`leading-6 ${isUser ? 'text-white' : 'text-foreground'}`}>{children}</li>
          ),
          ...
          // Enhanced sup/sub consistency
          sup: ({ children }) => (
            <sup className="align-super text-[0.8em] leading-none text-primary">
              {children}
            </sup>
          ),

          sub: ({ children }) => (
            <sub className="align-sub text-[0.8em] leading-none text-muted-foreground">
              {children}
            </sub>
          ),
