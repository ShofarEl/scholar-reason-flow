...
- Debug Results: ${JSON.stringify(debug.debugResults, null, 2)}`;
  ...
  const cancelCurrentBatch = async () => {
    ...
    setIsCanceling(true);
    try {
      const res = await BatchAPIService.cancelBatch(batchId);
      toast({ title: 'Cancel Requested', description: res.message });
    } catch (error) {
      toast({ title: 'Cancel Failed', description: error instanceof Error ? error.message : 'Unable to cancel batch', variant: 'destructive' });
    } finally {
  ...
  const handleGenerateProject = async () => {
    ...
    try {
      // Extract and store section titles from outline
      const titles = outline.chapters.map(chapter => chapter.title);
      setSectionTitles(titles);
      
      console.log(`🚀 Starting batch generation for ${titles.length} sections`);
      console.log(`🎯 Target: ${titles.length * 2500} words total`);
          ...
          (status) => {
            ...
            if (typeof status.actualWordCount === 'number') {
              setBatchStats({
                ...
                totalTokens: 0,
                targetWords: status.targetWordCount || titles.length * 2500,
      ...
      // Store raw sections and compute failed indices
      setSectionContents(sections);
      const failedIdx = sections
        .map((content, index) => ({ index, content }))
        .filter(x => x.content.startsWith(`# Section ${x.index + 1}: Error`) || x.content.includes('[Error processing section:'))
        .map(x => x.index);
      setFailedIndices(failedIdx);
      ...
      setBatchStats({
        ...
        targetWords: titles.length * 2500,
            ...
            <CardTitle className="flex items-center space-x-2">
              ...
              {batchId && (
                <div className="ml-auto flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={cancelCurrentBatch} disabled={isCanceling}>
              ...
              <div className="space-x-2">
                {failedIndices.length > 0 && (
                  <Button
                    ...
                    onClick={() => {
                      setIsRetrying(true);
                      toast({ title: 'Retry Requested', description: `${failedIndices.length} failed section(s) will be queued in a new batch.` });
                      // This is a non-breaking placeholder: real retry wiring can be added to submit a new batch for failedIndices using existing service
                      setTimeout(() => setIsRetrying(false), 800);
                    }}
                  >
                    {isRetrying ? 'Retrying…' : `Retry Failed (${failedIndices.length})`}
                  </Button>
                )}
