...
serve(async (req) => {
  ...
  const isCancelBatch = req.method === 'POST' && segments[segments.length - 1] === 'cancel' && segments.length >= 3 && segments[segments.length - 3] === 'batch';
  const getTrailingActionBatchId = () => segments[segments.length - 2];
  ...
  try {
    ...
    if (isPostBatch) {
      ...
      const batchId = getTrailingActionBatchId();
      console.log(`🛑 Cancel request for batch: ${batchId}`);

      try {
        const cancelResponse = await fetch(`https://api.anthropic.com/v1/messages/batches/${batchId}/cancel`, {
          method: 'POST',
          headers: {
            'x-api-key': CLAUDE_API_KEY,
            'anthropic-version': '2023-06-01',
        ...
        const cancelDataText = await cancelResponse.text();
        let cancelData: any = null;
        try {
          cancelData = JSON.parse(cancelDataText);
        } catch {
          cancelData = { raw: cancelDataText };
        }
        ...
        if (!cancelResponse.ok) {
          return new Response(JSON.stringify({
            success: false,
            status: 'error',
            message: `Cancel failed: ${cancelResponse.status} ${cancelResponse.statusText}`,
            details: cancelData
          }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        ...
        return new Response(JSON.stringify({
          success: true,
          status: cancelData?.processing_status || 'canceling',
